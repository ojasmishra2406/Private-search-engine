...
class DenseIndexer:
    ...
    Used by the IncrementalIndexer daemon to coordinate dense updates alongside lexical updates.
    """
    ...
    def add_batch(self, ext_ids: List[str], texts: List[str]):
        """Encodes texts in batches and adds them to the vector index."""
        if not ext_ids or not texts or len(ext_ids) != len(texts):
            return
            
        # Process in chunks of self.batch_size
        for i in range(0, len(ext_ids), self.batch_size):
            batch_ids = ext_ids[i : i + self.batch_size]
    ...
    def remove_batch(self, ext_ids: List[str]):
        """Removes the given IDs from the vector index."""
        self.vector_index.remove_vectors(ext_ids)
